...
double Tnew[(nx*ny)+1],Told[(nx*ny)+1],b[(nx*ny)+1],Mp[(nx*ny)+1],Mnw[(nx*ny)+1],Mw[(nx*ny)+1],Mn[(nx*ny)+1],Me[(nx*ny)+1],Ms[(nx*ny)+1],Mse[(nx*ny)+1];
double Lw[(nx*ny)+1],beta,Lp[(nx*ny)+1],Ls[(nx*ny)+1],Un[(nx*ny)+1],Ue[(nx*ny)+1],dx,dy;int N;
double SIP();
double output_files();
double analytic();
...
double output_files()
{
	...
	y1=ny/2;
	y2=(ny/2)-1;
	fprintf( output3, "VARIABLES=\"X\",\"code_Temp\",\"analytical_Temp\"\n");
	for(i=1;i<=nx;i++)
	{
		L1=(i-1)*ny+y1;
		avg1=Tnew[L1];
